...
# Timeout generoso: la primera llamada tras arrancar Ollama puede
# tardar varios segundos porque tiene que cargar el modelo en memoria.
...
def embed_text(text: str) -> list[float]:
    ...
    Returns:
        Lista de floats con la dimensión propia del modelo.
...
def embed_batch(
    ...
    Reutiliza un único cliente HTTP para amortizar el overhead de conexión.

    Args:
        texts: Lista de textos. Cada uno debe ser no-vacío.
        batch_size: Tamaño del lote enviado a Ollama en cada request.
    ...
    with httpx.Client(timeout=_DEFAULT_TIMEOUT) as client:
        for batch_idx in range(total_batches):
            ...
            if not isinstance(vectors, list) or len(vectors) != len(batch):
                raise EmbeddingError(
                    f"Respuesta inesperada de Ollama: se esperaban {len(batch)} "
                    f"embeddings, se recibió: {data!r}"
                )
...
def embedding_dimension() -> int:
    ...
    """
    vector = embed_text("dimension probe")
    return len(vector)
